...
def main():
    ...
    with col4:
        telefonista = st.selectbox("Telefonista", ["Exclusivo","Central", "Propio"])
        nodo_central = st.radio("¿Es nodo o central abastecimiento domicilios?", ["Sí", "No"], horizontal=True)
    ...
    with col7:
        ubicacion = st.selectbox("Ubicación", ["Zona comercial", "Barrio", "Vía principal"])
    with col8:
        pass
    ...
    with col9:
        entrega_merc   = st.radio("Entrega de mercancía", ["Punto de venta", "Bodega central"], horizontal=True)
        DIAS_SEMANA = ["Lunes", "Martes", "Miércoles", "Jueves", "Viernes", "Sábado"]
        dias_pedido = st.multiselect(
            "Días de pedido",
            options=DIAS_SEMANA,
            placeholder="Seleccione uno o más días"
        )
        dias_visita = st.multiselect(
            "Días de visita",
            options=DIAS_SEMANA,
            placeholder="Seleccione uno o más días"
        )
    ...
    with col10:
        frecuencia_ped = st.selectbox("Frecuencia de pedido", ["Diario", "Semanal", "2 veces x sem", "3 veces x sem"])
    ...
    for item in ITEMS_CAJA:
        with st.expander(f"📦 {item}", expanded=False):
            c1, c2 = st.columns(2)
            with c1:
                tiene = st.radio("¿Tiene?", ["Sí", "No"], key=f"caja_tiene_{item}", horizontal=True)
            with c2:
                cant = st.number_input("Cantidad", min_value=0, step=1, key=f"caja_cant_{item}")
    ...
    for item in ITEMS_PDV:
        with st.expander(f"🏪 {item}", expanded=False):
            c1, c2 = st.columns(2)
            with c1:
                tiene = st.radio("¿Tiene?", ["Sí", "No"], key=f"pdv_tiene_{item}", horizontal=True)
            with c2:
                cant = st.number_input("Cantidad", min_value=0, step=1, key=f"pdv_cant_{item}")
            if item == "Otros":
                obs = st.text_input("¿Cuál? (observaciones)", key=f"pdv_obs_{item}", placeholder="Ej: Botadero")
            else:
                obs = st.text_input("Observaciones", key=f"pdv_obs_{item}", placeholder="Opcional")
    ...
    for item in ITEMS_VIS:
        with st.expander(f"👁 {item}", expanded=False):
            c1, c2 = st.columns(2)
            with c1:
                tiene = st.radio("¿Tiene?", ["Sí", "No"], key=f"vis_tiene_{item}", horizontal=True)
            with c2:
                cant = st.number_input("Cantidad", min_value=0, step=1, key=f"vis_cant_{item}")
    ...
    if st.button("✅ Enviar Solicitud", type="primary", use_container_width=True):
        ...
        payload = {
            ...
            "dias_de_pedido": ", ".join(dias_pedido),   # "Lunes, Miércoles, Viernes"
