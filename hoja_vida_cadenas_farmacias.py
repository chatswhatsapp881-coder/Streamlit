...
def main():
        ...
        if pdv_info and pdv_sel != _pdv_anterior:
            ...
            if ean_pdv:
                ...
                if so_agg["tiene_datos"]:
                    # Inyectar en session_state ANTES del rerun
        ...
        if _cadena_val:
            for _i, _opt in enumerate(_cadenas_opts):
                if _opt.lower() == _cadena_val.lower():
                    _cadena_idx = _i
                    break
        nombre_cadena   = st.selectbox("Nombre de la cadena", _cadenas_opts, index=_cadena_idx)
    ...
    # Inicializar session_state solo si no existe (no sobreescribir lo inyectado por rerun)
    ...
    # Siempre resolver ventas desde session_state si ya fue inyectado, sino calcular
    ...
    # Pre-llenar marcas: prioridad session_state (inyectado por rerun) > hoja_vida > sellout
    ...
    if not _any_marca_loaded:
        # Intentar desde hoja_vida guardada
    ...
    # Siempre sincronizar session_state con los valores resueltos
    for i in range(1, 6):
    ...
    # Pre-llenar SKUs: prioridad session_state (inyectado por rerun) > hoja_vida > sellout
    ...
    # Datos previos de sell_out:
    # Prioridad: hoja_vida guardada > sellout calculado desde BD
    _so_prev = _hv.get("sell_out") or {}
    if _so_prev.get("mensual"):
        # Usar datos de hoja de vida guardada
        _so_mensual_prev = {r["mes"]: r for r in _so_prev["mensual"]}
    elif _so.get("sell_out_mensual"):
        # Usar datos calculados desde tabla sellout
    ...
    for mes in MESES:
        ...
        # Inicializar session_state solo si no existe (preserva valores inyectados por rerun)
    ...
    def _buscar_marca_so(nombre_formulario: str, indice_so: dict) -> dict:
        """Busca en el índice de sellout por coincidencia parcial del nombre."""
        nombre_up = nombre_formulario.upper()
        # Primero búsqueda exacta
        if nombre_up in indice_so:
            return indice_so[nombre_up]
        # Luego búsqueda parcial (el nombre del formulario contiene parte del nombre en BD)
